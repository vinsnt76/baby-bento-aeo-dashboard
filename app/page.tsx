...
import { BASELINE_DEC_25 } from '@/app/baseline-dec-25';
import AEOView from '@/app/AEOView';
import ClassicSEOView from '@/app/ClassicSEOView';
...
export default function BabyBentoDashboard() {
  ...
  const { resetView, selectedStartDate, selectedEndDate, setReportPeriod } = useStore();
  const lastUpdated = BASELINE_DEC_25.snapshotMonth;

  return (
    <div
      className="min-h-screen bg-[#2A1A5E] bg-fixed font-sans text-[#2D334A] pb-20"
      style={{
        backgroundImage:
          'radial-gradient(at top left,#2A1A5E 0%,transparent 50%), radial-gradient(at bottom right,#0F4C75 0%,transparent 50%)',
      }}
    >
        ...
        <header className="bg-[#F8F5F1] text-[#2D334A] p-8 shadow-lg rounded-xl flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs font-bold tracking-widest text-[#FF6F61] mt-1 uppercase">
                AEO Intelligence Engine
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center p-2 gap-2">
            <button
              ...
              className={`flex-1 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${
                activeTab === 'aeo'
                  ? 'bg-[#FDDF5B] text-black shadow-xl scale-[1.02]'
