...
export async function POST(req: Request) {
  try {
    ...
    // Zero-Data Check: Prevent hallucination on empty datasets
    const density = parseFloat(metrics.semantic_density);
    const ownership = parseFloat(metrics.ownership_score);
    
    if (density === 0 && ownership === 0) {
      return NextResponse.json({
        insights: ["Insufficient data for semantic analysis. Please broaden your date range to capture entity signals."],
        nextBestActions: ["Broaden Date Range", "Verify GSC Data Connection"],
        confidence: 0.0
      });
    }
      ...
      **Output Requirements:**
      ...
      - Keys: 
        ...
        - "nextBestActions" (string[]): Provide exactly 2 recommendations:
            ...
            - **Constraint:** Keep titles under 40 characters to prevent UI crowding.
